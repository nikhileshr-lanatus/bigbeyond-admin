...
import { createPaymentForArtistUsingStripe, getAllPaymentRequests, updateArtistPaymentStatusForOrderData } from '../api/payment';
...
const PaymentRequests = () => {
    ...
    const columns = [
        ...
        },
        {
            ...
            renderCell: (params) => {
                return ((params.row.isPaymentRequested) &&
                    <Typography
                        onClick={async () => {
    ...
    const payArtistApiStripeCallOnPayNow = async (data, allData) => {
        setLoading(true);
        try {
            const createPayment = await createPaymentForArtistUsingStripe(data, authToken)
            if (createPayment.status === 200) {
                showSnackBarNotification(
                    "success",
                    "Payment done for artist",
                    5000
                )
                await markArtistPaidForOrders(
                    allData?.dueOrdersList
                )
            } else {
                showSnackBarNotification(
                    "error",
    ...
    const getPayments = async () => {
        getAllPaymentRequests(authToken).then((res) => {
            setPaymentsData(res?.data.map((item) => {
                const b = item?.users
                const artistId = b?.id
                delete b.id
